    ...
    <div class="row">
      ...
      <div style="width: 60%; float: left; padding-left: 2px; padding-right: 2px;">
        <table class="table table-bordered table-striped">
              <thead>
                <tr>
                  <td colspan="8" align="center">
                    <span style="width: 70%">Assigned Task</span>
                    </td>
                </tr>
              </thead>
              <tbody>
              <?php
                $q = "SELECT * FROM tbl_task WHERE taskuser = '".$_SESSION["email"]."' or taskassign = '".$_SESSION["email"]."' order by docno, date_time desc";
                $con = mysqli_connect('localhost', 'root', '', 'lvmgt');
                $result_task = mysqli_query($con, $q);
              ...
              <tr>
                <form action="profile.php" method="post" role="form">
                  <table class="table table-striped table-bordered">
                    <tr>
                      <td rowspan="2"><span><p><?php echo $row['docno'];?></p></span><span><?php echo $row['date_time'];?></span></td>
                      <?php
                        if($row['taskassign'] == $_SESSION['email']){
                          echo "<td> Assigned To:";
                          $query_name = "select fn_username('".$row['taskuser']."') as taskassign from tbl_task where docno = '".$row['docno']."'";
                          $con = mysqli_connect('localhost', 'root', '', 'lvmgt'); 
                          $result_name=mysqli_query($con, $query_name); 
                          while ($row_name = mysqli_fetch_array($result_name)) {
                            echo $row_name['taskassign'];
                          }
                        }
                        if($row['taskassign'] != $_SESSION['email']){
                          echo "<td> Assigned By:";
                          $query_name = "select fn_username('".$row['taskassign']."') as taskassign from tbl_task where docno = '".$row['docno']."'";
                          $con = mysqli_connect('localhost', 'root', '', 'lvmgt'); 
                          $result_name=mysqli_query($con, $query_name); 
                          while ($row_name = mysqli_fetch_array($result_name)) {
                            echo $row_name['taskassign'];
              ...
              <td rowspan="2" align="center"><input type="submit" name="doneSubmit" value="Done">
                    </tr>
                    <tr>
                      <td colspan="2">Title:<?php echo $row['tasktitle'];?></td>
              <td style="display: none;"><input type="text" name="ketToEdit" value="<?php echo $row['docno']?>"></td>
                    </tr>
                    <tr>
                      <form action="profile.php" method="post" role="form">
                      <td style="display: none;"><input type="text" name="ketToEdit" value="<?php echo $row['docno']?>"></td>
                      <?php
                        $q_comment = "select * from tbl_task_comment where docno = '".$row['docno']."' ";
                        $con = mysqli_connect('localhost', 'root', '', 'lvmgt');
                        $result_cmt = mysqli_query($con, $q_comment);

                        
                        while ($row = mysqli_fetch_array($result_cmt)) {
                      ?>
                          <td colspan="4">  <?php echo $row['comment'];?> </td><tr>

                      <?php
                        }
                      ?><tr>
                      <td colspan="4"><input maxlength="1000" type="text" name="comment" placeholder="comment" style="width: 90%"><span><input type="submit" name="submitComment" value="Submit"></span></td>  
                      </form>
                    </tr>
                  </table>
                </form>    
